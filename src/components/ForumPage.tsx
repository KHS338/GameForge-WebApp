...
import { forumsApi, type ApiForumComment, type ApiForumTopic, type ApiGame } from '../api';
...
function getTopicSourceLabel(topic: ApiForumTopic, games: Pick<ApiGame, '_id' | 'title'>[]) {
  return games.find((item) => item._id === topic.gameId)?.title ?? 'Unknown game';
}

export function ForumPage({ games, initialGameId = null }: ForumPageProps) {
  const [browseGameId, setBrowseGameId] = useState<string>(initialGameId ?? '');
  const [createGameId, setCreateGameId] = useState<string>(initialGameId ?? games[0]?._id ?? '');
  ...
  const activeGame = useMemo(
    () => games.find((game) => game._id === browseGameId) ?? null,
    [games, browseGameId],
  );
  ...
  async function loadTopics() {
    ...
    try {
      ...
      const data = browseGameId
        ? await forumsApi.getGameTopics(browseGameId, topicQuery)
        : await forumsApi.getAllTopics(topicQuery);
  ...
  }, [browseGameId, topicSort, topicSearch]);
  ...
  async function handleCreateTopic(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const created = await forumsApi.createTopic(createGameId, {
  ...
  return (
    <article className="panel forum-panel">
      <div className="section-heading compact">
        <div>
          <span className="eyebrow">Game forums</span>
          <h2>{selectedTopic ? selectedTopic.title : activeGame?.title ? `${activeGame.title} topics` : 'All topics'}</h2>
        </div>
      </div>
      ...
      {!selectedTopic ? (
        <>
          <div className="forum-controls">
            {games.length > 0 && (
              <select value={browseGameId} onChange={(event) => setBrowseGameId(event.target.value)}>
                {games.map((game) => (
          ...
          <div className="forum-topics-list" style={{ maxHeight: 'none' }}>
            ...
            <>
              <header className="forum-thread-header">
                <div>
                  ...
                  <p>
                    by {selectedTopic.authorId?.username ?? 'Unknown'} · {toDate(selectedTopic.createdAt)} · {getTopicSourceLabel(selectedTopic, games)}
                  </p>
                </div>
